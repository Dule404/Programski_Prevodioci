...
//identifikatori
({slovo}|_)({slovo}|{dcifra}|_)* { return new Symbol(sym.ID, yytext() ); }

//konstante
{dcifra}+ { return new Symbol( sym.INTCONST, new Integer( yytext() )  ); }
0#(o{ocifra}+|x{hcifra}+|d{dcifra}+) { return new Symbol( sym.INTCONST, new Integer( yytext() ) ); }
0\.{dcifra}+(E(\+|-){dcifra}+) { return new Symbol( sym.FLOATCONST, new Float( yytext() )  ); }
